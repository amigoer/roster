...
- **只有清单的**：ACP agent 只需要三样东西——命令、协议→环境变量名和启动参数、登录方式——加一个 npm 依赖（agent 程序本身）。清单只按预设 id 写它特别对待的那几家；其余预设看它说自己会哪些协议，会就能接，地址从预设目录里取。几十种 agent 里大多数是这种：**加一个 agent 是写一份清单，不是写代码。** 下一批的现状和通道要补的几件事见 [接入更多 ACP agent](acp-agents.md)。
