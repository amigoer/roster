...
> 第一轮 · 2026-09-12 · 最近改动 2026-09-20（接入 OpenCode、DeepSeek Harness、Kimi Code；清单能写死环境变量、声明预设和工具分类、钉住模式；权限只答一次性的选项）
...
```
Bot → Agent → Harness（Claude Code / Codex / Gemini CLI / Grok Build / OpenCode / DeepSeek Harness / Kimi Code / pi-agent）
...
| ACP agent（Codex、Gemini CLI、Grok Build、OpenCode、DeepSeek Harness、Kimi Code、自定义命令） | ACP，按清单把协议或预设映射到环境变量，要启动时才定的模型也放进去 | ACP |
