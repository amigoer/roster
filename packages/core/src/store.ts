...
export const UNTITLED = /^(与 .+ 的会话|New conversation|新群聊)$/;
...
export class Store {
  ...
  backlog(member: MemberRow): { items: TranscriptItem[]; upTo: number } {
    ...
    for (const r of rows) {
      ...
      const text =
        e.type === "assistant.text" ? e.delta : e.type === "human.text" || e.type === "system.notice" ? e.text : "";
  ...
  private project(
    ...
    switch (event.type) {
      ...
      case "assistant.text": {
        ...
        this.#setPreview(conversationId, name ? `${name}：${preview}` : preview);
        return [msg];
      }
      case "tool.start":
      case "tool.end": {
        ...
        this.db
          ...
          .run(JSON.stringify(body), t, card.id);
      ...
      case "permission.decision": {
        ...
        this.db
          ...
          .run(event.decision.action === "allow" ? "allowed" : "denied", t, row.id);
